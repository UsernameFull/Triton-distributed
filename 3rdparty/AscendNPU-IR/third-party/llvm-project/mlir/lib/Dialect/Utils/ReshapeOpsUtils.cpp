...
static bool isAllOnes(const SmallVector<int64_t> &shape) {
  return llvm::all_of(shape, [](auto x) { return x == 1; });
}
...
bool mlir::areLooseReassociationsCompatible(
  ...
  if (sourceShape.empty()) {
    return isAllOnes(resultShape);
  }
  if (resultShape.empty()) {
    return isAllOnes(sourceShape);
  }
